...
void tcpConn2() {
    ...
    printf("Upon sending all packets to the server: Follower3 has TCP port %d and IP address %s for Phase 2 \n",ntohs(foo.sin_port), inet_ntoa(foo.sin_addr));
    ...
    string following;
    string f1liked;
    following = isFollowing("Follower3.txt");
    f1liked = followerLiked("Follower3.txt");

    send(socket_fd,following.c_str(),MAXDATA,0);

    send(socket_fd,f1liked.c_str(),MAXDATA,0);

    cout << "Completed sending feedback for <Follower3>" << endl;
    cout << "End of Phase 2 : End of Phase 2 for <Follower3>" << endl;
...
void udpConn() {
    ...
    if ((rv = getaddrinfo(LOCAL, u3, &hints, &servinfo)) != 0) {
        fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
        exit(1);
    }
    ...
    cout << "Upon start of Phase 2: <Follower3> has UDP port "<<u3<<" and IP address "<<ip_client<<endl;
    ...
    cout << "<Follower3> has received <Tweet"<<data_rev[0]<<">"<< endl;
    // recvfrom(sockfd,data_rev,sizeof data_rev,0,(struct sockaddr *)&their_addr, &addr_len);
    // cout << "<Follower3> has received <Tweet"<<data_rev[0]<<">"<< endl;
    // recvfrom(sockfd,data_rev,sizeof data_rev,0,(struct sockaddr *)&their_addr, &addr_len);
    // cout << "<Follower3> has received <Tweet"<<data_rev[0]<<">"<< endl;
