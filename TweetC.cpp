...
void tcpConn1(){
    ...
    printf("Upon startup of Phase 1: TweetC has TCP port %d and IP address %s for Phase 1 \n",ntohs(foo.sin_port), inet_ntoa(foo.sin_addr));

    printf("Upon establishing a TCP connection to the server: TweetC is now connected to the Server\n");


    //retrive tweet from tweet#.txt
    vector<string> tweets;
    ifstream ifile("TweetC.txt");
    string fileLine;
    ...
    for(int m=0;m<tweets.size();m++){
        ...
        cout << "Sending tweet information to the server: TweetC has sent "<< tosend.substr(1) << " to server \n";
    }

    
    printf("Upon sending all the tweets’ information to the server: Updating the server is done for TweetC  \n");
    printf("End of Phase 1: End of Phase 1 for TweetC\n");
...
void tcpConn2(){
    ...
    printf("Upon startup of Phase 2: TweetC has TCP port %d and IP address %s for Phase 2 \n",ntohs(foo.sin_port), inet_ntoa(foo.sin_addr));
    ...
    cout << "End of phase 2 End of phase 2 for TweetC" << endl;
}
